...
To demonstrate PIPs I created a shellcode (shown in Listing 5.4) for the MIPS and X86 architectures using existing platform specific shellcodes by Richard Imrigan[@Imrigan:vg] and TheWorm[@TheWorm:vp].  The shellcode uses a single PIP header which would also allow this PIP to be valid for the ARM architecture if it were extended further.

````{ basicstyle=\tt, mathescape=true, caption="An example of a shellcode PIP for X86 and MIPS which attempts to spawn a shell and elevate permissions.  Shellcode for each architecture was taken from \autocite{Imrigan:vg}\autocite{TheWorm:vp}." }
...
Clearly the shellcode is far from ideal.  A decompilation of the PIP header can be seen in Table 5.7
